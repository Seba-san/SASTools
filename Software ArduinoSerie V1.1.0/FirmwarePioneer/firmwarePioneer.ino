...
bool band_tx = 0, band_control = 0, band_rx = 0,
...
void serialEvent() {
    ...
    if (encabezado < 2) {
        ...
        i = i + 1;
        // if(i>=(2+2*cantidad_actuadores)){
        if (i >= (4 + 2 * cantidad_actuadores)) {
            // if(inData[1+2*cantidad_actuadores]==0){
            if (inData[3 + 2 * cantidad_actuadores] == 0) {

                //  for(k=0; k<(2*cantidad_actuadores+1);k++){
...
void loop() {
    ...
    if (band_control == 1) {
        ...
        if (band_tx == 1) {
            ...
            // envio[0]=envio[0]& 0x3FFF; //DEBUG
            envio[0] = envio[0] & 0x7FFF; // DEBUG

            if (promedio < 0) {
                // envio[0]=envio[0] | 0x4000; //agrega signo en bit 15  DEBUG
            ...
            // envio[1]=envio[1]& 0x3FFF; //DEBUG
            envio[1] = envio[1] & 0x7FFF;

            if (yw < 0) {
                // envio[1]=envio[1] | 0x4000; //agrega signo en bit 15 DEBUG
            ...
            envio[2] =
                (vel2_envio * 2 * 117.02857); //   //convierte float a uint 14
                                              //   bits+1 signo  REVISAR
            // envio[2]=envio[2]& 0x3FFF; //DEBUG
            envio[2] = envio[2] & 0x7FFF;

            if (veloc2f < 0) {

                // envio[2]=envio[2] | 0x4000; //agrega signo en bit 15 DEBUG
            ...
            envio[3] =
                (vel1_envio * 2 * 117.02857); //   //convierte float a uint 14
                                              //   bits+1 signo REVISAR
            // envio[3]=envio[3]& 0x3FFF; //DEBUG
            envio[3] = envio[3] & 0x7FFF;

            if (veloc1f < 0) {

                // envio[3]=envio[3] | 0x4000; //agrega signo en bit 15 DEBUG
            ...
            // envio[4]=envio[4]& 0x3FFF;//DEBUG
            envio[4] = envio[4] & 0x7FFF;

            if (uv < 0) {
                // envio[4]=envio[4] | 0x4000; //agrega signo en bit 15 DEBUG
            ...
            envio[5] =
                (uw_envio * 2 *
                 682.66); //   //convierte float a uint 14 bits+1 signo REVISAR
            // envio[5]=envio[5]& 0x3FFF; //DEBUG
            envio[5] = envio[5] & 0x7FFF;

            if (uw < 0) {
                // envio[5]=envio[5] | 0x4000; //agrega signo en bit 15 DEBUG
                envio[5] = envio[5] | 0x8000;
            }

            ////////// corriente motor 1 y 2 ///////////
            envio[6] = id1;
            envio[7] = id2;
            ////////////////////////////////////////////////
        ...
        if (band_rx == 1) {
            ...
            // rv=(float)256.00*(0x3F & inData1[2])+inData1[1];   //referencia
            // de velocidad lineal byte a float
            rv = (float)256.00 * (0x7F & inData1[4]) + inData1[3];
            // rw=(float)256.00*(0x3F & inData1[4])+inData1[3];   // de 14 bits
            // + 1 signo  DEBUG
            rw = (float)256.00 * (0x7F & inData1[6]) + inData1[5];

            // if((inData1[2]& 0x40)>0){                // verifica signo en bit
            // 15  DEBUG
            if ((inData1[4] & 0x80) > 0) {
                signo[0] = 1;
                rv = rv * (-1.00);
            ...
            // if((inData1[4]& 0x40)>0){                // verifica signo
            // velocidad angular en bit 15 DEBUG
            if ((inData1[6] & 0x80) > 0) {
                signo[1] = 1;
                rw = rw * (-1.0);
            }
