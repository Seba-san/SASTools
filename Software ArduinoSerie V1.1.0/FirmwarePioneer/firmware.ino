...
Posibles errores ver $3
Posibles mejoras ver $4

Este código es para un Arduino Mega.

Enciende el LED del pin 38 cuando esta en MODO=1 y lo apaga cuando esta en
MODO=0.

MODO=0 : No alimenta los motores, apaga led.

MODO=1 : Hace funcionar los motores con el PID. Tiene un PID para la velocidad angular y otro para la velocidad lineal. 

Comunicacion:
        Recibe asincrónicamente:
    [FF FF][modo,2,8][signed int vel_lineal_referencia, signed int
vel_angular_referencia]

        Transmite periodicamente:
[FF
FF][modo,vel_lineal,vel_angular,vel_izq,vel_der,u_lin,u_angular,corriente_izq,corriente_der][00]
son todos bytes con signo.
...
#define base_line 32     // cm
#define radio_rueda 8.25 // cm
...
bool evento_tx = 0, evento_control = 0, evento_rx = 0,
     signo[2]; // banderas de control y de transmision

char buffer[200];

float angulo = 0, angulo2 = 0, veloc_angular1 = 0, veloc_angular2 = 0, i1 = 0,
      i2 = 0, dif1[10];
float promedio = 0, diferencia = 0, veloc1 = 0, veloc2 = 0, veloc1f = 0,
      veloc2f = 0, veloc1f_1 = 0, veloc2f_1 = 0;

// Variables PID Lineal
float y, rv, u, u2, uv, y1, u1,
    u22; // señal,referencia, acción de control  y1(k)=y(k-1)
float Kp, Ki, Kd, deriva, Td; // Parametros PID + Td<1
float integral, integral1;
float e, e1, ef, ef1, d1,
    d; // error, termino derivativo  e1=e(k-1)  d1(k)=d(k-1)

// Variables PID Angular
float yw, rw, uw = 0, u2w,
              y1w;        // señal,referencia, acción de control  y1(k)=y(k-1)
float Kpw, Kiw, Kdw, Tdw; // Parametros PID + Td<1
float integralw, integral1w;
float ew, e1w, d1w, dw, efw,
    efw1; // error, termino derivativo  e1=e(k-1)  d1(k)=d(k-1)

unsigned int pwm1, pwm2, modo = 0;
byte inData[10];
byte inData1[10];

float prom_envio = 0, dif_envio = 0, uv_envio = 0, uw_envio = 0, vel1_envio = 0,
      vel2_envio = 0;
...
void serialEvent() {
    /* Trama RX:
    [FF FF][modo,n_actuadores,n_sensores][2x n_actuadores]
    [FF FF][modo,n_actuadores,n_sensores][2byte+sgn vel_lineal, 2byte+sgn
vel_angular] Recibe cada byte verificando el encabezado y la cantidad de datos
que se esperan (n_actuadores) escritos en 2 bytes. Una vez que se recibieron
todos los datos, levanta la bandera evento_rx y se lee en el bucle principal. De
forma estatica, deberia ser: n_actuadores=2 (ref angular y lineal en 2 bytes)
                n_sensores=8

    Esta rara esta funcion. $3
    */
...
//---------------- ------------------
// $2 Subrutinas de interrupcion y funciones
...
void setup() {
  ...
  // ############ MAIN $1
void loop() {
    if (evento_control == 1) {       // Bandera de interrupcion por timer 1
        veloc1 = cuenta1 * 5.124735; // relacion 360/9850*1000ms=36.54 , 1°=
                                     // 0.017rad , 8.25cm=radio de rueda
        veloc2 = cuenta2 * 5.124735;
        veloc1f =
            veloc1f_1 * 0.99 + veloc1 * 0.01; // filtra velocidad  rueda derecha
        ...
        diferencia =
            (57.3 / 32) *
            (veloc1f -
             veloc2f); // velocidad angular en grad/seg; 57.3=360/2pi; L=32Cm

        y = promedio;
        yw = diferencia;
        //------EMANUEL
        VmotorIzq = analogRead(A14) * (5.0 / 1023.0); // lectura del sensor
        ImotorIzq = (VmotorIzq - 2.5) *
                    Sensibilidad; // Ecuación  para obtener la corriente
        VmotorDer = analogRead(A15) * (5.0 / 1023.0); // lectura del sensor
        ImotorDer = (VmotorDer - 2.5) *
                    Sensibilidad; // Ecuación  para obtener la corriente
        //----------------------------
        if (modo == 1) { // Modo PID
            //#### Pid velocidad lineal
            digitalWrite(led, HIGH);
            e = rv - y; // calculo del error, rv= Valor de referencia

            ef = ef1 * 0.9 + e * 0.1; // Error filtrado
            ef1 = ef;
            // Calculo de integral
            if ((e > 30) || (e < -30)) {
                integral = 0;
            ...
            if ((e < 30) && (e > -30)) {
                integral =
                    integral1 + ((Ts * 0.5) * (e + e1)); // e1 error en k-1
            }
            ...
            y1w = yw;
            // Calculo velocidades angulares ruedas
            // Revisar... $3
            uw = uw * 0.08; //??
            u = (uv + uw);  // derecha
            u2 = (uv - uw); // izquierda
            ...
            pwm1 = (unsigned int)((float)u1 *
                                  (1023.0 / TENSION_MAX)); // u para derecho
            pwm2 = (unsigned int)((float)u22 *
                                  (1023.0 / TENSION_MAX)); // u2 para izquierdo
            // PWM
            if (modo > 0) {
        ...
        /* Trama:
                [FF FF]
                [modo,vel_lineal,vel_angular,vel_izq,vel_der,u_lin,u_angular,corriente_izq,corriente_der][00]
                1 byte por variable.
        */
        // $4 Meter en una funcion.
        if (evento_tx == 1) {
            ...
            envio[0] =
                (prom_envio * 2 * 117.02857); // convierte float a uint 14
                                              //   bits+1 signo REVISAR $3
            // Le pone signo al entero
            ...
            envio[1] =
                (dif_envio * 2 *
                 81.92); //   //convierte float a uint 14 bits+1 signo   REVISAR
            ...
            envio[2] =
                (vel2_envio * 2 * 117.02857); //   //convierte float a uint 14
                                              //   bits+1 signo  REVISAR
            ...
            envio[3] =
                (vel1_envio * 2 * 117.02857); //   //convierte float a uint 14
                                              //   bits+1 signo REVISAR
            ...
            envio[4] =
                (uv_envio * 2 *
                 682.66); //   //convierte float a uint 14 bits+1 signo REVISAR
        ...
        /*
Trama:
[modo,n_actuadores_n_sensores,referencia_lineal,referencia_angular]
-70<rv<-5, 5<rv<70
Tener en cuenta que cada referencia son 2 bytes.
        */
        if (evento_rx == 1) {
            modo = inData1[0];                // Recibe modo
            cantidad_actuadores = inData1[1]; // Redundante? $4
